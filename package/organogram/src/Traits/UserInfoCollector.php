...
// change path from App to a2i\organogram to reference models
use a2i\organogram\database\Models\Menu;
use a2i\organogram\database\Models\MenuRoleMap;
use App\Models\Office;
use a2i\organogram\database\Models\OfficeFrontDesk;
use App\Models\OfficeUnit;
use App\Models\OfficeUnitOrganogram;
use App\Models\ProtikolpoManagement;
use a2i\organogram\database\Models\User;
use a2i\organogram\database\Models\XSetting;
use Carbon\Carbon;
...
trait UserInfoCollector
{
    ...
    public function getAssignedMenus()
    {
        ...
        $menus = collect();
        if ($user_role_id == config('organogrampkg.super_admin')) {
            $role_menu = $this->getUserRoleMenu($user_role_id);
            $menus = $menus->merge($role_menu);
        }
        if ($user_role_id == config('organogrampkg.admin')) {
            $role_menu = $this->getUserRoleMenu($user_role_id);
            $menus = $menus->merge($role_menu);

        }
        if ($user_role_id == config('organogrampkg.user')) {
            if ($user_office_roles) {
                if ($user_office_roles->is_unit_admin == 1 && $user_office_roles->is_admin == 0) {
                    $role_menu = $this->getUserRoleMenu(config('organogrampkg.unit_admin'));
                    $menus = $menus->merge($role_menu);
                }
                if ($user_office_roles->is_unit_head == 1) {
                    $role_menu = $this->getUserRoleMenu(config('organogrampkg.unit_head'));
                    $menus = $menus->merge($role_menu);
                }
                if ($user_office_roles->is_office_head == 1) {
                    $role_menu = $this->getUserRoleMenu(config('organogrampkg.office_head'));
                    $menus = $menus->merge($role_menu);
                }
                if ($user_office_roles->is_admin == 1) {
                    $role_menu = $this->getUserRoleMenu(config('organogrampkg.office_admin'));
                    $menus = $menus->merge($role_menu);
                }
                if ($isFrontDesk == 1) {
                    $role_menu = $this->getUserRoleMenu(config('organogrampkg.office_frontdesk'));
                    $menus = $menus->merge($role_menu);
                }
            }
        }
