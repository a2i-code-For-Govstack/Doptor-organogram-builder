<div class="col-md-12">
    @foreach ($office_units as $office_unit)
        <table class="table table-bordered">
            <thead>
            <tr class="group">
                <td colspan="2" style="font-weight:bold;">{{ $office_unit->unit_name_bng }}</td>
            </tr>
            </thead>
            <tbody>
            @foreach ($office_unit->active_organograms as $organogram)
                <tr id="checked_unit_4604_org_10007" role="row" class="check_row">
                    <td style="width: 40%;">
                        <input @if ($organogram->assigned_user) disabled="true" checked="true" @endif
                            ...
                            <div class="row_inside_input" id="checked_input_field_{{ $organogram->id }}">
                                <div class="input-group">
                                    ...
                                    <input id="designation_level{{ $organogram->id }}" type="hidden"
                                           name="designation_level" value="{{ $organogram->designation_level }}">
                                    ...
                                    <input id="joining_date{{ $organogram->id }}" name="joining_date"
                                           type="text" class="form-control rounded-0 assingDate"
                                           placeholder="দিন-মাস-বছর"
                                           aria-label="Recipient's username with two button addons"
                                           aria-describedby="button-addon4">
                                    <select class="custom-select rounded-0 user-options"
